    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] p-6 space-y-1">
          ...
          <div className="text-3xl font-bold text-white" suppressHydrationWarning>
            {mounted ? `$${totalValue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '—'}
          </div>
          <div className={`text-sm font-semibold ${totalPnl >= 0 ? 'text-[var(--color-success)]' : 'text-[var(--color-danger)]'}`} suppressHydrationWarning>
            {mounted ? `${fmt$(totalPnl)} (${totalPnlPct >= 0 ? '+' : ''}${totalPnlPct.toFixed(2)}%)` : '—'}
          </div>
        </div>
      ...
      {mounted && iv && iv.history.length > 0 && (
